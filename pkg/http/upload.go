...
func uploadLinkHandler(w http.ResponseWriter, r *http.Request, d *common.Data) (int, error) {
	...
	if strings.HasPrefix(path, upload.CacheRequestPrefix) {
		...
		path = proxy.RewriteUrl(path, cachePvc, upload.CachePathPrefix, false)
	} else {
		if strings.HasPrefix(path, "/data") {
			path = "/data" + proxy.RewriteUrl(strings.TrimPrefix(path, "/data"), userPvc, "", false)
		} else {
			path = proxy.RewriteUrl(path, userPvc, "", false)
		}
	}
...
func uploadedBytesHandler(w http.ResponseWriter, r *http.Request, d *common.Data) (int, error) {
	...
	if strings.HasPrefix(parentDir, upload.CacheRequestPrefix) {
		...
		parentDir = proxy.RewriteUrl(parentDir, cachePvc, upload.CachePathPrefix, false)
	} else {
		if strings.HasPrefix(parentDir, "/data") {
			parentDir = "/data" + proxy.RewriteUrl(strings.TrimPrefix(parentDir, "/data"), userPvc, "", false)
		} else {
			parentDir = proxy.RewriteUrl(parentDir, userPvc, "", false)
		}
	}
