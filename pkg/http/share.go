...
func shareLinkDownloadHandler(w http.ResponseWriter, r *http.Request, d *common.Data) (int, error) {
	...
	klog.Infof("~~~Debug log: originalPath=%s, fullPath=%s", originalPath, fullPath)

	fileParam, _, err := UrlPrep(r, fullPath)
	if err != nil {
		return http.StatusBadRequest, err
	}
	...
	downloadPath := uri + fileParam.Path

	file, err := files.NewFileInfo(files.FileOptions{
		Fs:         files.DefaultFs,
		Path:       strings.TrimPrefix(downloadPath, "/data"),
		Modify:     true,
		Expand:     false,
	...
	return drives.RawDirHandler(w, r, d, file)
