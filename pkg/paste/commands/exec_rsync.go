...
import (
	...
	"files/pkg/constant"
	"files/pkg/fileutils"
	storageExec "files/pkg/storage/executor"
	storageRsync "files/pkg/storage/rsync"
	"files/pkg/utils"
	"fmt"
	"strings"
...
func (c *Command) rsync() error {
	...
	_, err = storageRsync.ExecRsync(c.ctx, rsync, args, c.UpdateProgress)
...
func (c *Command) move() error {
	...
	if _, err = storageExec.ExecCmd(context.Background(), mv, args); err != nil {
		return err
	}
