...
func (s *CloudStorage) Rename(contextArgs *models.HttpContextArgs) ([]byte, error) {
	...
	s.service.Stat(contextArgs.FileParam, "", "")

	var srcPrefixPath = gerRenamedSrcPrefixPath(fileParam.Path)

	stat, err := s.service.Stat(contextArgs.FileParam, srcPrefixPath, dstName)
	if err != nil {
		klog.Errorf("Cloud rename, user: %s, stat error: %v", owner, err)
		return nil, err
	}

	klog.Infof("Cloud rename, user: %s, isSrcFile: %v, srcPrefixPath: %s, stat: %s", owner, isSrcFile, srcPrefixPath, utils.ToJson(stat))

	if stat != nil && stat.Item != nil {
