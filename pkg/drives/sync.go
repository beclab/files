...
func (rs *SyncResourceService) GeneratePathList(db *gorm.DB, processor PathProcessor) error {
	for bflName, cookie := range common.BflCookieCache {
		...
		for _, repo := range data.Repos {
			...
			url := "http://127.0.0.1:80/seahub/api/v2.1/repos/" + repo.RepoID + "/dir/?p=/&with_thumbnail=true"
			klog.Infoln(url)
			...
			for dirent := range generator {
				err = processor(db, dirent.Drive, dirent.Path, dirent.Mtime)
				if err != nil {
...
func walkSyncDirentsGenerator(body []byte, header *http.Header, r *http.Request, repoID string) <-chan DirentGeneratedEntry {
	...
	go func() {
		...
		for len(queue) > 0 {
			...
			fullPath := filepath.Join(SrcTypeSync, repoID, firstItem.Path)
