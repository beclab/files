...
func (m *MediaEncoder) SetFFmpegPath() bool {
	...
	if ffmpegPath == "" {
		...
		fmt.Println("GetEncodingOptions: ", m.configurationManager.GetEncodingOptions())
		fmt.Println("EncoderAppPath", m.configurationManager.GetEncodingOptions().EncoderAppPath)
	...
	if m.ffmpegPath != nil {
		...
		if runtime.GOOS == "linux" &&
			...
			fmt.Println("VaapiDevice: ", options.VaapiDevice)
...
func (m *MediaEncoder) GetInputPathArgument(state mediaencoding.EncodingJobInfo) string {
	fmt.Println("oooooooooooooooooooooooooooooooooooo-------------------", state.MediaPath)
...
func (m *MediaEncoder) GetMediaInfoInternal(
	...
	*/
	...
	if err := cmd.Start(); err != nil {
		fmt.Println("Start Errrrrrrrrrrrrr")
		fmt.Println(err)
		return nil, err
	}
	...
	if _, err := io.Copy(&memoryBuffer, stdout); err != nil {
		cmd.Process.Kill()
		fmt.Println("Copy Errrrrrrrrrrrrr")
		fmt.Println(err)
		return nil, err
	}

	if err := cmd.Wait(); err != nil {
		fmt.Println("Wait Errrrrrrrrrrrrr")
		fmt.Println(err)
		return nil, err
	}

	fmt.Println(string(memoryBuffer.Bytes()))
