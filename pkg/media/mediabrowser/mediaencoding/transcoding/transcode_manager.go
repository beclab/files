...
func NewTranscodeManager(mediaEncoder mediaencoding.IMediaEncoder, fileSystem ioo.IFileSystem, serverConfigurationManager configuration.IServerConfigurationManager, sessionManager cs.ISessionManager) *TranscodeManager {
	t := &TranscodeManager{
...
func (m *TranscodeManager) GetTranscodingJob2(path string, jobType mediaencoding.TranscodingJobType) *mediaencoding.TranscodingJob {
	...
	defer m.activeTranscodingJobsLock.RUnlock()
	for _, job := range m.activeTranscodingJobs {
		fmt.Println(path, *job.Path, job.Type, jobType)
	...
	var jobs []*mediaencoding.TranscodingJob
	{
		...
		for _, job := range m.activeTranscodingJobs {
			if playSessionID == "" {
				...
				fmt.Println("............PlaySessionID................")
				jobs = append(jobs, job)
			}
		}
	}

	fmt.Println("kill..................................job len ", len(jobs))
