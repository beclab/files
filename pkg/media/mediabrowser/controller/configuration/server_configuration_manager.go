...
func (s *ServerConfigurationManager) GetConfiguration(string) any {
	klog.Infoln("ServerConfigurationManager.GetConfiguration###############")
	return `{
...
func (s *ServerConfigurationManager) GetEncodingOptions() *configuration.EncodingOptions {
	...
	if false {
		...
		if !ok {
			klog.Infoln("not ok GetEncodingOptions")
			return nil
		}
		klog.Infoln(options)
		return options
