...
func (t *TranscodingJob) StartKillTimerWithTimeout(callback func(interface{}), intervalMs time.Duration) {
	...
	if t.killTimer == nil {
		log.Printf("Starting kill timer at %dms. JobId %s PlaySessionId %s", intervalMs, *t.ID, *t.PlaySessionID)
		//	t.killTimer = time.AfterFunc(time.Duration(intervalMs)*time.Millisecond, func() {
		t.killTimer = time.AfterFunc(intervalMs, func() {
			callback(t)
		})
	} else {
		log.Printf("Changing kill timer to %dms. JobId %s PlaySessionId %s", intervalMs, *t.ID, *t.PlaySessionID)
...
func (t *TranscodingJob) ChangeKillTimerIfStarted() {
	...
	if t.killTimer != nil {
		...
		log.Println("Changing kill timer to %dms. JobId %s PlaySessionId %s", intervalMs, t.ID, t.PlaySessionID)
		t.killTimer.Reset(intervalMs)
	}
}

func (t *TranscodingJob) Stop() {
	fmt.Println("job stop...........................", *t.ID)
	t.processLock.Lock()
