...
func (t *TranscodingSegmentCleaner) Start() {
	fmt.Println("Starting TranscodingSegmentCleaner")
...
func (t *TranscodingSegmentCleaner) timerCallback(state interface{}) {
	fmt.Println("timerCallback-1 ", *t.job.ID)
	if t.job.HasExited {
		t.DisposeTimer()
		return
	}
	fmt.Println("timerCallback-2 ", *t.job.ID)
...
func (t *TranscodingSegmentCleaner) DeleteSegmentFiles(job *TranscodingJob, idxMin, idxMax int64, delayMs int) error {
	...
	fmt.Printf("Deleting segment file(s) index %d to %d from %s\n", idxMin, idxMax, *path)
	...
	case Hls:
		if err := t.DeleteHlsSegmentFiles(*path, idxMin, idxMax); err != nil {
			...
			fmt.Printf("error deleting segment file(s) %s: %v\n", *path, err)
			return err
		}
	default:
