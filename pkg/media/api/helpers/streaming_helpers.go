...
func GetStreamingState(
	...
	if !ok {
		fmt.Println("notttttttttttttttttttttttttttttttttt ok")
	}
	fmt.Printf("look        -->        %+v %+v\n", streamingRequest, ok)
	fmt.Println(streamingRequest.SegmentContainer)
	if streamingRequest.Params != nil && *streamingRequest.Params != "" {
		ParseParams(streamingRequest)
	}

	streamingRequest.StreamOptions = ParseStreamOptions(httpContext.URL.Query())
	if httpContext.URL.Path == "" {
		return nil, errors.New("resource not found")
	}

	urlRequest := filepath.Ext(httpContext.URL.Path)

	if streamingRequest.AudioCodec == "" {
		streamingRequest.AudioCodec = encodingHelper.InferAudioCodec(urlRequest)
	}

	userAgent := httpContext.Header.Get("User-Agent")
	...
	*/
	...
	/*
	   item, err := libraryManager.GetItemById(streamingRequest.Id)
	   if err != nil {
	       return nil, errors.New("resource not found")
	   }
	   state.IsInputVideo = item.MediaType == common.MediaType_Video
	*/
	...
	if protocol == mediaprotocol.Http {
		...
		fileParam, err := models.CreateFileParam(bflName, httpContext.URL.Query().Get("PlayPath"))
		if err != nil {
			log.Printf("parse url error: %v\n", err)
			return nil, errors.New("parse url error")
		}
		if fileParam.FileType == common.Sync {
			remoteAccessToken := httpContext.Header.Get("remote-accesstoken")
	...
	if streamingRequest.LiveStreamId == "" {
		...
		if mediaSource == nil {
			...
			fmt.Printf("____________________________%+v\n", mediaInfo)
			mediaSource = &mediaInfo.MediaSourceInfo

			/*
			   mediaSources, err := mediaSourceManager.GetPlaybackMediaSources(item, nil, false, false, ctx)
			   if err != nil {
			       return nil, err
			   }

			   if streamingRequest.MediaSourceId == "" {
			       mediaSource = mediaSources[0]
			   } else {
			       for _, source := range mediaSources {
			           if source.ID == streamingRequest.MediaSourceId {
			               mediaSource = source
			               break
			           }
			       }
			   ...
			   if mediaSource == nil && streamingRequest.MediaSourceId == streamingRequest.Id {
			       mediaSource = mediaSources[0]
			   }
			*/
		}
		...
		/*
		   liveStreamInfo, err := mediaSourceManager.GetLiveStreamWithDirectStreamProvider(streamingRequest.LiveStreamId, ctx)
		   if err != nil {
		       return nil, err
		   }
	...
	if state.VideoRequest != nil {
		...
		fmt.Println("outputVideoBitrate: ", outputVideoBitrate)
...
func GetOutputFilePath(state *streaming.StreamState, outputFileExtension string, serverConfigurationManager configuration.IServerConfigurationManager, deviceId, playSessionId string) string {
	...
	folder := serverConfigurationManager.GetTranscodePath()
	fmt.Println("GetOutputFilePath", filepath.Join(folder, filename+ext))
...
func ParseParams(r interface{}) {
	...
	fmt.Println(*videoRequest.Params)
