...
func GetKubernetesClient() (*kubernetes.Clientset, error) {
	...
	fmt.Println("Using controller-runtime configuration")
...
func WriteConfigMap(clientset *kubernetes.Clientset, namespace, name string, data map[string]string) error {
	...
	if apierrors.IsNotFound(err) {
		...
		fmt.Printf("Created ConfigMap %s in namespace %s (ResourceVersion: %s)\n", name, namespace, created.ResourceVersion)
		return nil
	}
	...
	fmt.Printf("Updated ConfigMap %s in namespace %s (ResourceVersion: %s)\n", name, namespace, updated.ResourceVersion)
