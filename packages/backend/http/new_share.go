...
var shareLinkGetHandler = withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	//outputHeader(r)
...
func checkPathAndConvertToPathID(path, ownerID string) (uint64, error) {
	...
	if pathInfo, exists := pathIDMap[path]; exists {
		...
		fmt.Printf("Path %s not found in database\n", path)
...
var shareLinkPostHandler = withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	if err != nil {
		fmt.Println("Error converting path to path ID:", err)
	...
	newShareLink := postgres.ShareLink{
		LinkURL:    host + "/share_link/" + stringMD5(r.URL.Path+fmt.Sprint(time.Now().UnixNano())),
		PathID:     pathID,
		Path:       r.URL.Path,
		Password:   stringMD5(requestBody.Password),
		OwnerID:    ownerID,
		OwnerName:  ownerName,
...
var shareLinkDeleteHandler = withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	//outputHeader(r)
